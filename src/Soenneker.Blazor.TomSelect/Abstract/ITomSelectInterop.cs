...
public interface ITomSelectInterop : IEventListeningInterop, IAsyncDisposable
{
    ...
    /// </summary>
    /// <param name="useCdn">Determines whether assets should be loaded from the CDN.</param>
    ...
    /// Initializes the TomSelect interop by loading required scripts and styles.
    /// </summary>
    /// <param name="configuration">Optional configuration settings that determine how Tom Select assets are loaded.</param>
    /// <param name="cancellationToken">A token to cancel the initialization operation.</param>
    /// <returns>A <see cref="ValueTask"/> representing the asynchronous operation.</returns>
    ValueTask Initialize(TomSelectConfiguration? configuration, CancellationToken cancellationToken = default);
